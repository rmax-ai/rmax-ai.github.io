...
- 2026-07-01: Published standalone page: [new-to-ai-fde](new-to-ai-fde/index.html). Introduces AI Forward Deployed Engineering for engineers and technical leaders through the role definition, core capabilities, operating model, representative workflows, and major challenges. Moved from `notes/` to its own root-level page (like `/start-here/`) with standalone styling, back-link navigation, and the same 3 Mermaid diagrams.
  - *Change*: Moved from `/notes/new-to-ai-fde/` to `/new-to-ai-fde/` — removed from notes index, updated homepage CTA, regenerated sitemap.
