...
- 2026-07-01: Published technical note: [new-to-ai-fde](notes/new-to-ai-fde/index.md). Introduces AI Forward Deployed Engineering for engineers and technical leaders through the role definition, core capabilities, operating model, representative workflows, and major challenges.
  - *Warnings*: Publish with caution — the role framing can be over-applied outside mature enterprise environments; the governance model assumes baseline ownership, review capacity, and rollback discipline even though the note now scopes itself to consequential workflows.
- 2026-06-30: Published technical note: [ai-fde-organizational-learning-systems](notes/ai-fde-organizational-learning-systems/index.md). Argues that AI Forward Deployed Engineering teams should operate as organizational learning systems that turn field deployments into reusable platform capabilities, patterns, and durable local ownership.
  - *Warnings*: Publish with caution — the note can be over-applied as a general operating model for teams that lack repeated deployments, platform ownership, or mature review functions; several normative claims may be read as best practice rather than a scoped lab artifact; the argument depends on prerequisites such as cross-team evidence capture, durable ownership, and the ability to compare deployments by problem class; some teams may adopt the learning-system machinery too early and add governance overhead before recurrence is stable; operational readers may over-centralize review loops or artifact production and reduce local judgment if safeguards remain informal; tooling and practice references around prompts, orchestration, and platformization may age faster than the core organizational claim.
- 2026-06-29: Published technical note: [ai-native-workflow-redesign](notes/ai-native-workflow-redesign/index.md). Presents a framework for redesigning workflows for AI-native execution around scoped authority, validation, exception handling, and operational controls.
  - *Warnings*: Publish with caution — readers may treat the framework as generally authoritative rather than bounded, the framework assumes substantial organizational and technical maturity, and named controls and exception paths are not equivalent to operationally real control; release with follow-up fixes — the note was linked from home and notes indexes, the sitemap entry for the new note was still pending regeneration in Step 8, and two pre-existing insecure `http` outbound links were observed in `index.html` cards: `http://mcp-auth-test-server.rmax.tech` and `http://agentic-workflows.rmax.ai`.
