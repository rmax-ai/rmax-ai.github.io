...
### Show your work (reasoning & trade-offs)

**Assumptions**

* Your notes are already concept-dense and one-idea-per-file.
* Links are an *interface*, not decoration.

**Why subagents**

* Prevents the orchestrator from hallucinating coherence.
* Forces each link to survive a single-axis critique.

**Trade-offs**

* Slower than brute-force embedding similarity.
* Much higher signal and long-term PKB integrity.
...
* `agent-execution-contracts.md` ↔ `earned-agent-autonomy.md`
* Structural-agent validates this as `extends` because one defines mechanics, the other governance thresholds.

**Example 2 — Temporal**

* Early note on “LLMs as kernels” ↔ later note on “Harness optimization”
* Temporal-agent confirms conceptual maturation, proposes `historical-context`.
