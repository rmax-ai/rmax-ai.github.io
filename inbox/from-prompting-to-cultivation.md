...
The dominant mental model for working with AI agents has been *projection*: encoding our own mental frameworks into prompts and expecting agents to execute like apprentices. A more powerful paradigm is emerging—*cultivation*. In this model, we declare goals first, grant agents real agency, observe their behavior, and then iteratively design the environment—tools, processes, constraints—that allows them to thrive. This shift reframes engineering from instruction to habitat design. Systems like **Gas Town**, described by Steve Yegge, make this transition explicit: leverage comes not from better prompts, but from better environments.
...
At scale, projection leads to diminishing returns. You end up debugging your own thinking twice—once in your head, once in the prompt.
...
Cultivation treats agents as semi-autonomous actors embedded in an environment. Performance emerges from the interaction between:
...
Gas Town demonstrates that the future of engineering is not "AI writes all the code," but "humans design ecosystems where code-writing agents can operate productively."
...
Reference: https://steve-yegge.medium.com/the-future-of-coding-agents-e9451a84207c
