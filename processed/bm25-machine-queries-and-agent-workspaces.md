...
Both experimental conditions share the same corpus, BM25 index, chunking policy, model, task set and total token ceiling.
...
As of this draft, the repository contains the experimental contract, core implementation and passing offline tests, but no live-model benchmark result suitable for a publication claim. The project is evidence generation, not evidence theatre.
...
First, evaluate retrieval on the queries your system actually issues. Human benchmark queries are a poor proxy if production traffic consists of model-generated subqueries, copied error messages and iterative tool calls.

Second, separate retrieval recall from task success. A system can retrieve every required document and still fail to combine the evidence. It can also start with weak recall and recover through subsequent searches.

Third, treat the interface as an experimental variable. Comparing BM25 with a vector database while simultaneously changing chunking, context size, tool access and model prompts produces an architecture comparison that cannot explain its own result.

Fourth, match total budgets. An iterative agent with eight model turns should not be compared casually with a one-call baseline. Tool use consumes context, output tokens and time even when the local search operation itself is cheap.

Finally, preserve exact-search paths even in dense systems. Code symbols, policy identifiers, transaction references and version strings are not edge cases in enterprise software. They are often the shortest route to the evidence.
...
- Hongjin Su et al. [BRIGHT: A Realistic and Challenging Benchmark for Reasoning-Intensive Retrieval](https://arxiv.org/abs/2407.12883), ICLR 2025.
...
- Liang Wang et al. [Query2doc: Query Expansion with Large Language Models](https://arxiv.org/abs/2303.07678), EMNLP 2023.
- rmax.ai. [BM25 VFS Workspace vs BM25 Snippet Ablation](https://github.com/rmax-ai/bm25-vfs-ablation), 2026.
