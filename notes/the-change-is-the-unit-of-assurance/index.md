---
title: "The Change Is the Unit of Assurance"
slug: "the-change-is-the-unit-of-assurance"
description: "AI-generated software needs a persistent evidence model that survives the pull request: the software change itself — claims, evidence, risk, provenance, deployment state, and runtime observations — as the unit of assurance, not the diff."
author: "Max"
site: "rmax.ai"
section: "notes"
type: "note"
...
date: "2026-09-05"
updated: "2026-09-05"
tags: ["assurance", "software-engineering", "ai-agents", "provenance", "delivery"]
reading_time: "16–18 min"
canonical_url: "https://rmax.ai/notes/the-change-is-the-unit-of-assurance/"
license: "CC BY 4.0"
---
...
[DORA's research](https://dora.dev/ai/gen-ai-report/) describes the resulting tension: AI can increase author velocity while transferring cognitive load to reviewers, and faster code generation can produce larger batches, slower review, and greater instability. DORA: [Balancing AI tensions](https://dora.dev/insights/balancing-ai-tensions/) · DORA: [Impact of Generative AI in Software Development](https://dora.dev/ai/gen-ai-report/)
...
flowchart TD
    ...
    subgraph DEF["definition"]
        I["intent"]
        C["behavioral claims"]
        B["affected boundaries"]
        V["invariants"]
        R["risk"]
    end
    subgraph EXE["execution & observation"]
        IMP["implementation"]
        E["evidence"]
        D["deployment state"]
        RO["runtime observations"]
...
NASA's Software Assurance and Software Safety Standard describes assurance, safety, and independent verification and validation across the software lifecycle, including operations, maintenance, and retirement. [NASA-STD-8739.8B](https://standards.nasa.gov/standard/NASA/NASA-STD-87398)

NIST's Secure Software Development Framework likewise includes preparation, software protection, secure production, and response to vulnerabilities discovered after release. It recommends tracking requirements, risks, design decisions, and provenance rather than treating security as a final check. [NIST Secure Software Development Framework](https://csrc.nist.gov/pubs/sp/800/218/final)

Assurance cases provide the conceptual model: a structured argument connecting claims about a system to evidence and assumptions. [ISO/IEC/IEEE 15026-2:2022](https://www.iso.org/standard/80625.html) applies this model to both developing and maintaining assurance cases.

The [OMG Structured Assurance Case Metamodel](https://www.omg.org/spec/SACM/) makes such arguments machine-representable and supports evidence including tests, measurements, records, expert judgments, and automatically collected artifacts.
...
**software generation throughput > assurance throughput**

DORA's research captures this at the review layer: creation and verification are different cognitive tasks, and AI-generated changes can turn author velocity into reviewer load. DORA: [Balancing AI tensions](https://dora.dev/insights/balancing-ai-tensions/)
...
**proposed ↓ specified ↓ implemented ↓ verified ↓ deployed progressively ↓ observed ↓ accepted, remediated, rolled back, or superseded**
...
The implementation realizes these claims, but the claims are closer to what the organization needs to know. This is the central idea behind assurance cases: confidence is organized around claims and evidence, not artifact inspection alone. ([ISO](https://www.iso.org/standard/80625.html))
...
> **Weak:** Improve payment reliability.
>
> **Stronger:** Under transient PSP failures, retryable payment attempts eventually succeed or fail without creating duplicate captures.
>
> **Operational:** During canary and full rollout, duplicate captures remain zero, payment success rate does not decline, and p99 latency increases by less than 5% relative to the control population.
...
Google's SRE guidance addresses this with canarying: expose a change to a bounded subset of production, evaluate it, and use the results to decide whether rollout should continue. [Google SRE: Canarying Releases](https://sre.google/workbook/canarying-releases/)
...
**proposed ↓ supported by analysis ↓ verified under test ↓ validated under bounded production exposure ↓ validated under sustained operation**

At any point:

**claim falsified ↓ rollback / remediation / new investigation**
...
Runtime verification provides a formal precedent: execution traces can be evaluated against specifications while a system is running rather than only before deployment. [Survey of Runtime Verification](https://arxiv.org/abs/1811.06740)

Recent research similarly explores updating confidence in safety arguments with runtime evidence. [Runtime confidence updates in safety arguments, SAC 2026](https://arxiv.org/abs/2605.22530)

The general pattern is:

**design-time evidence + runtime evidence ↓ updated assurance state**
...
```mermaid
flowchart TD
    P["proposed"] --> RFR["ready_for_review"]
    RFR --> V["verified"]
    V --> AFC["approved_for_canary"]
    AFC --> CAN["canarying"]
    CAN --> AFR["approved_for_rollout"]
    AFR --> OBS["observing"]
    OBS --> ACC["accepted"]
    ACC --> SUP["superseded"]
    CAN --> BLK["blocked"]
...
- **proposed** — claims and risk recorded
- **ready_for_review** — required design and evidence plan present
- **verified** — pre-deployment evidence satisfies policy
- **approved_for_canary** — rollback, monitoring, and exposure limits configured
- **canarying** — bounded production evidence being collected
- **approved_for_rollout** — canary gates satisfied
- **observing** — full deployment complete; post-deployment window active
- **accepted** — required observation period complete
- **blocked** — required evidence missing or contradictory
- **falsified** — a claim violated or a critical assumption failed
- **rolled_back** — exposure reduced or removed
- **superseded** — a later change replaces the current behavior
...
For example, **verified → approved_for_canary** might require:
...
And **canarying → approved_for_rollout** might require:
...
Imagine that every consequential change receives a persistent identifier: `change://payments/retry-policy/4831`.
...
SLSA defines provenance as verifiable information about where, when, and how an artifact was produced. [SLSA Provenance](https://slsa.dev/spec/v1.0/provenance)

in-toto records metadata for supply-chain steps and establishes a chain that can later be validated. [in-toto: Getting Started](https://in-toto.io/docs/getting-started/)
...
```mermaid
flowchart TD
    INT["intent"] --> CL["claims"]
    CL --> SRC["source revision"]
    SRC --> ART["build artifact"]
    ART --> DEP["deployment"]
    DEP --> POP["runtime population"]
    POP --> OBS["observations"]
    OBS --> DEC["decision"]
```
...
At merge time:
We have evidence suggesting this migration is unlikely to
materially increase checkout failures under the conditions
we tested.
...
The claim has been falsified under a previously untested
condition.
```

The software did not become nondeterministic. Our knowledge changed.

Canarying and DevSecOps models make this lifecycle explicit: evidence collection, deployment monitoring, operation, and feedback continue after build and release. [Google SRE](https://sre.google/workbook/canarying-releases/) [NIST DevSecOps reference model](https://pages.nist.gov/nccoe-devsecops/notational-reference-model.html)
...
**potential consequence × novelty × blast radius × uncertainty × irreversibility**
...
This allows a reviewer to move from **claim ↓ argument ↓ evidence ↓ critical implementation** rather than from **diff ↓ guess what matters**.
...
**create change ↓ select risk tier ↓ generate claim template ↓ attach standard evidence ↓ configure rollout and rollback ↓ deploy with monitoring**
...
```mermaid
flowchart TD
    CO["Persistent change object"]
    CO --> PRE
    CO --> OPS
    subgraph PRE["definition & pre-merge"]
        SPEC["specification"]
        IMP["implementation"]
        CIE["CI evidence"]
        SECE["security evidence"]
    end
    subgraph OPS["deployment & operation"]
        DEP["deployment state"]
        RTE["runtime evidence"]
        INC["incidents / residual risk"]
    end
```
...
**write ↓ review ↓ test ↓ merge**

toward:

**intent ↓ claims ↓ implementation ↓ evidence ↓ bounded execution ↓ runtime evidence ↓ updated confidence ↓ continuous observation**
...
1. Create a durable record for each consequential change — before implementation is considered complete.
2. State intent and behavioral claims before reviewers inspect the implementation.
3. Link each important claim to evidence and to the conditions under which it was collected.
4. Mark every important claim as supported, not yet supported, or not applicable.
5. Make rollout, monitoring, rollback, and incident response update the same record.
6. Use risk tiers to determine how much evidence and human review are required.
7. Enforce the record through delivery controls: missing claim → block approval, missing rollback → block canary, missing runtime monitor → block rollout.
8. Start with one high-impact workflow end to end — not a universal assurance ontology.

## Positioning Note

Claims, evidence, assurance cases, runtime verification, canary deployments, provenance, and continuous monitoring are established practice. The proposed contribution is their combination under a single software-engineering abstraction: the consequential software change as a persistent assurance object whose claims, evidence, risk, provenance, deployment state, runtime observations, and residual uncertainty evolve throughout its lifecycle. That is a design argument rather than an empirical result.

## Status & Scope

This article describes a conceptual operating model, not a product specification. The record formats, state names, and risk tiers are illustrative starting points, not standards.

The model is intended for consequential, medium- and high-risk changes. Applying this machinery to routine, reversible edits would be wasteful — the goal is assurance that is proportional, persistent, and operationally useful.

The approach assumes an organization can enforce delivery controls. Without enforcement, the change record becomes documentation rather than assurance infrastructure.

## References

1. DORA — [Balancing AI tensions: Moving from AI adoption to effective SDLC use](https://dora.dev/insights/balancing-ai-tensions/) — 2025 State of AI-assisted Software Development research on AI's tradeoffs across the SDLC, including the verification tax and shifting reviewer load.
2. DORA — [Impact of Generative AI in Software Development](https://dora.dev/ai/gen-ai-report/) — research report on how gen AI adoption affects delivery throughput, delivery stability, and developer trust.
3. NASA — [NASA-STD-8739.8B: Software Assurance and Software Safety Standard](https://standards.nasa.gov/standard/NASA/NASA-STD-87398) — standard covering software assurance, safety, and independent verification and validation across the lifecycle, including operations, maintenance, and retirement.
4. NIST — [Secure Software Development Framework (SSDF), SP 800-218](https://csrc.nist.gov/pubs/sp/800/218/final) — practices for secure software development, including responding to post-release vulnerabilities and tracking requirements, risks, and design decisions.
5. ISO/IEC/IEEE — [15026-2:2022, Systems and software assurance — Part 2: Assurance case](https://www.iso.org/standard/80625.html) — requirements for the structure and terminology of assurance cases, applicable to developing and maintaining them.
6. OMG — [Structured Assurance Case Metamodel (SACM)](https://www.omg.org/spec/SACM/) — machine-representable assurance arguments with evidence including tests, measurements, records, expert judgments, and automatically collected artifacts.
7. Google — [SRE Workbook: Canarying Releases](https://sre.google/workbook/canarying-releases/) — partial, time-limited production deployment and evaluation as the basis for rollout decisions.
8. Sánchez et al. — [A Survey of Challenges for Runtime Verification from Advanced Application Domains](https://arxiv.org/abs/1811.06740) — runtime verification as the dynamic analysis of execution traces against formal specifications.
9. Herd, Kelly, Heinemann & Zacchi — [A Subjective Logic-based Method for Runtime Confidence Updates in Safety Arguments (SAC 2026)](https://arxiv.org/abs/2605.22530) — updating confidence in safety arguments with windowed runtime evidence.
10. SLSA — [Provenance specification v1.0](https://slsa.dev/spec/v1.0/provenance) — verifiable information about where, when, and how a software artifact was produced.
11. in-toto — [Getting Started](https://in-toto.io/docs/getting-started/) — layouts, link metadata, and verification establishing a continuous, validatable supply-chain record.
12. NIST NCCoE — [DevSecOps Practices: Notional Reference Model](https://pages.nist.gov/nccoe-devsecops/notational-reference-model.html) — a reference model for DevSecOps phases including deploy, operate, and continuous monitoring with automated evidence collection.
